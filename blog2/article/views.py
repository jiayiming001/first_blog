...
def detail(request, pk):
    ...
    article.article = md.convert(article.article)
    article.toc = md.toc
    form = CommentForm()
...
def search(request):
    key = request.GET.get('keyboard')
    error_msg = ''
    if key == '请输入关键词':
        error_msg = "请输入关键字"
        return render(request, 'index.html', {'error_msg':error_msg})
    artilce_list = Article.objects.filter(Q(title__icontains=key) | Q(article__icontains=key))
    return render(request, 'index.html', {'error_msg':error_msg,
                                          'article_list':artilce_list})
